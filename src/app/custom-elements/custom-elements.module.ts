...
export class CustomElementsModule {
  ...
  constructor(private elementsLoader: ElementsLoader) {
    for (const key of this.elementsLoader.getElementsToLoad().keys()) {
      if (document.getElementsByTagName(key).length > 0) {
        console.log(`${key} was found`);
        this.elementsLoader.loadCustomElement(key);
        console.log(`${key} was loaded`);
      }
    }
  }
}
